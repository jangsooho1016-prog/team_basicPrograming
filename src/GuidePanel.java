...
public class GuidePanel extends JPanel {
    ...
    public GuidePanel(CrazyArcade_UI mainFrame) {
        ...
        // [이미지 로드]
        // res/game play.png 리소스를 파일 시스템 기반 경로로 로드합니다.
        String guidePath = System.getProperty("user.dir") + File.separator + "res" + File.separator + "game play.png";
        File guideFile = new File(guidePath);
        if (guideFile.exists()) {
            ImageIcon icon = new ImageIcon(guidePath);
            guideImage = icon.getImage();
        } else {
            System.err.println("GuidePanel: 이미지 로드 실패 - " + guidePath);
